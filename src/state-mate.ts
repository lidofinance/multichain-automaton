...
export function setupStateMateConfig(configName: string, newContractsCfg: any, networkType: NetworkType) {
  ...
  item("l1OpStackTokenRatePusher", l1SectionEntries).value = newContractsCfg["ethereum"]["opStackTokenRatePusherImplAddress"];
...
function domainSeparator(name: string, version: string, chainid: number, addr: string) {
  ...
  const encodedParams = ethers.AbiCoder.defaultAbiCoder().encode([ "bytes32", "bytes32", "bytes32", "uint256", "address" ], [ typeHash, hashedName, hashedVersion, chainid, addr ]);
